...
def lanjutT():
	...
	if len(fid_bteman) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/fbteman.txt')
			fid_bteman = []
	return 0
def lanjutG():
	global fid_bgroup
	if len(fid_bgroup) != 0:
		...
		if i.upper() == 'L':
			...
			os.remove(os.sys.path[0]+'/fgroup.txt')
			fid_bgroup = []
	return 0
def menu():
	tampil('''\rm
...
\rp###########################################################
#             \rb*ASSASSIN BRUTEFORCE FACEBOOK*\rp              #
# \rcBY\rc                                             X-K1R4   \rp#
# \rcContact\rk  Assassincyberteam@gmail.com                    \rp#
# \rcTeam\rk                     AssassinCyberTeam              \rp#
#    \rmJangan Gunakan Alat Ini Untuk Tujuan IllegaL         \rp#
###########################################################''')
	tampil('''\rk%s\n\rh1 \rbAmbil id dari group\n\rh2 \rbAmbil id dari daftar teman\n\rh3 \rmKELUAR\n\rk%s'''%('#'*20,'#'*20))
